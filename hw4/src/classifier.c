...
void gradient_matrix(matrix m, ACTIVATION a, matrix d)
{
    ...
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
            double x = m.data[i][j];
            // TODO: multiply the correct element of d by the gradient, use if-else statements for different activations
        }
    }
}
...
matrix forward_layer(layer *l, matrix in)
{
    ...
    matrix out = make_matrix(in.rows, l->w.cols); // change this
...
double l2_loss(matrix y, matrix p)
{
    // TODO
    return 0;
}
...
double l1_loss(matrix y, matrix p)
{
    // TODO
    return 0;
}
